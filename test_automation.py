    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if (input_by_ph.count() > 0 and output_by_ph.count() > 0
                    and input_by_ph.is_visible() and output_by_ph.is_visible()):
...
def _parse_args():
    ...
    # NEW: browser select option
    parser.add_argument(
        "--browser",
        default="chromium",
        choices=["chromium", "chrome", "edge", "firefox"],
        help="Browser to use: chromium, chrome, edge, firefox"
    )
    return parser.parse_args()

def _launch_browser(p, args):
    browser_choice = args.browser.lower()
    slow_mo = max(0, int(args.slow_mo_ms))
    headless = args.headless

    print(f"Launching browser: {browser_choice}")
...
def run_test():
    ...
    input_col_idx = _find_column_index(
        header_values, args.input_col, DEFAULT_INPUT_COLUMN_CANDIDATES
    )
    expected_col_idx = _find_column_index(
    ...
    actual_col_idx = _find_column_index(
        header_values, args.actual_col, DEFAULT_ACTUAL_COLUMN_CANDIDATES
    )
    status_col_idx = _find_column_index(
        header_values, args.status_col, DEFAULT_STATUS_COLUMN_CANDIDATES
    )

    actual_col_idx = actual_col_idx or _ensure_column(
        ws, header_row, header_values, actual_col_name
    )
    status_col_idx = status_col_idx or _ensure_column(
        ws, header_row, header_values, status_col_name
    )
    ...
    with sync_playwright() as p:
        ...
        if is_chat:
            try:
                input_locator, output_locator, action_locator = _find_chat_locators(
                    page, int(args.timeout_ms)
                )
            except Exception as e:
            ...
            output_locator = (
                ...
                .filter(has_text=re.compile(r"\bSinhala\b"))
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if prev_output and actual_output == "" and prev_output != "":
                    raise RuntimeError(
                        "Output did not update for this input (still showing previous output)."
                    )
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    try:
                        ...
                        print(f"Warning: Could not save after row {row_index}: {save_err}")
